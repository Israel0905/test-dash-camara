...
class JTT808Base(TCPUDPBase):
        ...
        try:
            if message:
                ...
                for i in range(0, range_num, 2):
                    msg = bytearray(msgs[msg_indexs[i]:msg_indexs[i + 1] + 1])
    ...
    def send(self, data, res_msg_id, serial_no):
        ...
        resp_res = {}
        count = 0
        _timeout = self._timeout * (count + 1)
        while count <= self._retry_count:
            send_res = self._send(data)
...
class JTT808(JTT808Base):
    ...
    def params_report(self, response_serial_no, terminal_params):
        ...
        if self._encryption:
            up_msg_obj.set_excryption(self._encryption, self._rsa_e, self._rsa_n)
        msgs = up_msg_obj.message()
        for serial_no, data in msgs:
