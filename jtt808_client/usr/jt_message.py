...
class JTMessage(object):
    _body_length_ = 0b0000001111111111
    _encryption_ = 0b0001110000000000
    _subpackage_ = 0b0010000000000000
    _version_ = 0b0100000000000000
    _reserved_ = 0b1000000000000000
    ...
    def __init__(self):
        ...
        self._jtt808_version = _jtt808_version
        ...
        self._properties = 0b0000000000000000
        if self.__version:
            self._properties |= self._version_
        else:
            self._properties ^= (self._properties & self._version_)

        self._properties = self._properties ^ (self._properties & self._encryption_) | (self.__encryption << 10)

        self._message_id = 0x0000
        self._serial_no = 0
    ...
    def __init_check_code(self, header, body):
        ...
        return str_fill(hex(check_code)[2:], target_len=2)
    ...
    def get_body_len(self):
        return self._properties & self._body_length_

    def set_body_len(self, body_len):
        self._properties = self._properties ^ (self._properties & self._body_length_) | body_len

    def get_encryption(self):
        return (self._properties & self._encryption_) >> 10
    ...
    def is_subpackage(self):
        return (self._properties & self._subpackage_) == self._subpackage_

    def set_subpackage(self, subpackage=False, package_total=0):
        if subpackage:
            self._properties |= self._subpackage_
            self._package_total = package_total
        else:
            self._properties ^= (self._properties & self._subpackage_)

    def is_version(self):
        return (self._properties & self._version_) == self._version_
    ...
    def header_to_hex(self):
        kwargs = {
            "message_id": str_fill(hex(self._message_id)[2:], target_len=4),
            "version": str_fill(hex(self._protocol_version)[2:], target_len=2) if self._protocol_version else self._protocol_version,
            "client_id": self._client_id,
            "package_total": "",
            "package_no": "",
        }
        logger.debug("header_to_hex: %s" % str(kwargs))
        if self.is_subpackage() and self._bodys:
            # Init properties
            for package_no in self._bodys.keys():
                body_len = int(len(self._bodys[package_no]) / 2)
                self.set_body_len(body_len)
                kwargs.update({
                    "serial_no": str_fill(hex(self._serial_no_obj.get_serial_no())[2:], target_len=4),
                    "properties": str_fill(hex(self._properties)[2:], target_len=4),
                    "package_total": str_fill(hex(self._package_total)[2:], target_len=4),
                    "package_no": str_fill(hex(package_no)[2:], target_len=4),
                })
                self._headers[package_no] = self.__splice_header(**kwargs)
        else:
            self.set_body_len(int(len(self._body) / 2))
            kwargs.update({
                "properties": str_fill(hex(self._properties)[2:], target_len=4),
                "serial_no": str_fill(hex(self._serial_no_obj.get_serial_no())[2:], target_len=4),
            })
            self._header = self.__splice_header(**kwargs)

    def init_check_code(self):
        if self.is_subpackage() and self._bodys:
            for package_no in self._bodys.keys():
                header = self._headers[package_no][1]
                body = self._bodys[package_no]
                self._check_codes[package_no] = self.__init_check_code(header, body)
        else:
            self._check_code = self.__init_check_code(self._header[1], self._body)
    ...
    def message(self):
        ...
        if self.is_subpackage() and self._bodys:
            msgs = [(self._headers[package_no][0], self.__message_to_hex(self._headers[package_no][1], self._bodys[package_no], self._check_codes[package_no])) for package_no in self._bodys.keys()]
            return msgs
        else:
            msg = self.__message_to_hex(self._header[1], self._body, self._check_code)
            return [(self._header[0], msg)]
    ...
    def set_header(self, header):
        self._message_id = header["message_id"]
        self._properties = header["properties"]
        self._protocol_version = header["protocol_version"]
        self._client_id = header["client_id"]
        self._serial_no = header["serial_no"]
        self._package_total = header["package_total"]
        self._package_no = header["package_no"]
    ...
    def body_subcontract(self):
        if self.is_subpackage():
            ...
            subpkg_len = int(int(len(self._body) / 2) / self._package_total)
            for i in range(1, self._package_total + 2):
                start_num = i * subpkg_len * 2
                end_num = (i + 1) * subpkg_len * 2
                if self._body[start_num:end_num]:
                    self._bodys[i] = self._body[start_num:end_num]
        logger.debug("is_subpackage: %s" % self.is_subpackage())
        logger.debug("__bodys: %s" % self._bodys)
    ...
    def rsa_encryption(self):
        if self.get_encryption():
            ...
            body_bytes = bytearray(ubinascii.unhexlify(self._body))
            if len(body_bytes) / 100 <= 1:
                self._body = ubinascii.hexlify(
                    rsa.encrypt(ubinascii.unhexlify(self._body), self.__server_pub_rsa_n, hex(self.__server_pub_rsa_e)[2:])
                ).decode()
            else:
                bodys = []
                ...
                self._body = "".join(bodys)
                self.set_subpackage(True, len(bodys))
            logger.debug("body after rsa_encryption:\n%s" % self._body)

    def rsa_decryption(self):
        if self.get_encryption():
            self._body = ubinascii.hexlify(rsa.decrypt(ubinascii.unhexlify(self._body))).decode()
...
class JTMessageParse(JTMessage):
    ...
    def __parse_check_code(self):
        self._check_code = int(self.__message[-2:], 16)

    def _check_code_check(self):
        ...
        for i in range(2, len(msgs)):
            check_code ^= msgs[i]
        if check_code != self._check_code:
            raise TypeError("check code is not compare. message check_code[%s], calculate check_code[%s]" % (self._check_code, check_code))

    def __parse_header(self):
        self._message_id = int(self.__message[:4], 16)
        self._properties = int(self.__message[4:8], 16)
        if self.is_version():
            self._protocol_version = int(self.__message[8:10], 16)
            self._client_id = self.__message[10:30]
            self._serial_no = int(self.__message[30:34], 16)
            if self.is_subpackage():
                self._package_total = int(self.__message[34:38], 16)
                self._package_no = int(self.__message[38:42], 16)
        else:
            self._client_id = self.__message[8:20]
            self._serial_no = int(self.__message[20:24], 16)
            if self.is_subpackage():
                self._package_total = int(self.__message[24:28], 16)
                self._package_no = int(self.__message[28:32], 16)

    def __parse_body(self):
        if self.is_version():
            if self.is_subpackage():
                self._body = self.__message[42:-2]
            else:
                self._body = self.__message[34:-2]
        else:
            if self.is_subpackage():
                self._body = self.__message[32:-2]
            else:
                self._body = self.__message[24:-2]
    ...
    def get_header(self):
        header = {
            ...
            "package_total": self._package_total,
            "package_no": self._package_no,
        }
        return header
...
class T0001(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{response_serial_no}{response_msg_id}{result_code}".format(**kwargs)
...
class T8001(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        self._body_data = {
            "serial_no": int(self._body[:4], 16),
            "message_id": int(self._body[4:8], 16),
            "result_code": int(self._body[8:10], 16),
        }
...
class T8004(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        time_item = [self._body[i * 2:(i + 1) * 2] for i in range(int(len(self._body) / 2))]
        self._body_data = {
...
class T8003(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        source_serial_no = int(self._body[:4], 16)
        if self.is_version():
            total_number = int(self._body[4:8], 16)
            pkids = self._body[8:]
        else:
            total_number = int(self._body[4:6], 16)
            pkids = self._body[6:]
...
class T0005(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{source_serial_no}{retransmission_num}{package_ids}".format(**kwargs)
...
class T0100(JTMessage):
    ...
    def set_params(self, province_id, city_id, manufacturer_id, terminal_model, terminal_id, license_plate_color, license_plate):
        ...
        elif self._jtt808_version == "2011":
            self.__manufacturer_id_len = 10
    ...
    def body_to_hex(self):
        ...
        self._body = "{province_id}{city_id}{manufacturer_id}{terminal_model}{terminal_id}{license_plate_color}{license_plate}".format(**kwargs)
...
class T8100(JTMessage):
    ...
    def body_from_hex(self):
        ...
        self._body_data = {
            "serial_no": int(self._body[:4], 16),
            "registration_result": int(self._body[4:6], 16),
            "auth_code": ubinascii.unhexlify(self._body[6:].encode("gbk")).decode("gbk"),
        }
...
class T0102(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{auth_len}{auth_code}{imei}{app_version}".format(**kwargs)
...
class T8103(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        param_count = int(self._body[:2], 16)
        param_body = self._body[2:]
        params = []
...
class T0104(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{serial_no}{param_count}{params}".format(**kwargs)
    ...
    def body_from_hex(self):
        ...
        body_data:
            ...
            cmd_word -- 2:
                ...
                conn_timeout(str): Time limit for connecting to the server, unit: minutes (default: "")
        """
        cmd_word = int(self._body[:2], 16)
        cmd_param = ""
        cmd_params = {}
        if cmd_word == 1:
            cmd_param = ubinascii.unhexlify(self._body[2:].encode("gbk")).decode("gbk")
...
class T0107(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{terminal_type}{manufacturer_id}{terminal_model}{terminal_id}{iccid}" \
...
class T8108(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        upgrade_type = int(self._body[:2], 16)
        manufacturer_id = ubinascii.unhexlify(self._body[2:12].encode("gbk")).decode("gbk")
        terminal_firmware_verion_len = int(self._body[12:14], 16)
        terminal_firmware_verion = ubinascii.unhexlify(self._body[14:(terminal_firmware_verion_len * 2 + 14)].encode("gbk")).decode("gbk")
        upgrade_package_len = int(self._body[(terminal_firmware_verion_len * 2 + 14):(terminal_firmware_verion_len * 2 + 22)], 16)
        upgrade_package = ubinascii.unhexlify(self._body[(terminal_firmware_verion_len * 2 + 22):(terminal_firmware_verion_len * 2 + 22 + upgrade_package_len * 2)].encode("gbk"))
        self._body_data = {
...
class T0108(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{upgrade_type}{result_code}".format(**kwargs)
...
class T0200(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{alarm_flag}{loc_status}{latitude}{longitude}{altitude}{speed}{direction}{time}{loc_additional_info}".format(**kwargs)
...
class T0201(JTMessage):
    ...
    def body_to_hex(self):
        ...
        logger.debug("kwargs: %s" % str(kwargs))
        self._body = "{response_serial_no}{alarm_flag}{loc_status}{latitude}{longitude}{altitude}{speed}{direction}{time}{loc_additional_info}".format(**kwargs)
...
class T8202(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        time_interval = int(self._body[:4], 16)
        location_tracking_validity_period = -1
        if time_interval != 0:
            location_tracking_validity_period = int(self._body[4:], 16)
        self._body_data = {
...
class T8203(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        alarm_msg_serial_no = int(self._body[:4], 16)
        alarm_type = str_fill(bin(int(self._body[4:], 16))[2:], target_len=32)
...
class T8300(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        flag = str_fill(bin(int(self._body[:2], 16))[2:], target_len=8)
        ...
        msg_type = int(self._body[2:4], 16) if self.is_version() else 0
        message = ubinascii.unhexlify(self._body[4 if self.is_version() else 2:]).decode("gbk")
        self._body_data = {
            "flag_type": flag_type,
...
class T8301(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        set_type = int(self._body[:2], 16)
        total_num = int(self._body[2:4], 16)
        event_body = self._body[4:]
        events = []
...
class T0301(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{event_id}".format(**kwargs)
...
class T8302(JTMessage):
    ...
    def body_from_hex(self):
        ...
        question_info_len = int(self._body[2:4], 16)
        question_info = ubinascii.unhexlify(self._body[4: 4 + question_info_len * 2]).decode("gbk")
        answers = []
        answer_body = self._body[4 + question_info_len * 2:]
        while answer_body:
            answer = {}
...
class T0302(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{response_serial_no}{answer_id}".format(**kwargs)
...
class T0303(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{info_type}{onoff}".format(**kwargs)
...
class T8304(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        info_type = int(self._body[:2], 16)
        info_len = int(self._body[2:6], 16)
        info_data = ubinascii.unhexlify(self._body[6:6 + info_len * 2]).decode("gbk")
        self._body_data = {
            "info_type": info_type,
            "info_data": info_data,
        }
...
class T8400(JTMessage):
    ...
    def __init__(self):
        super().__init__()
        self._message_id = 0x8400

    def body_from_hex(self):
        flag = int(self._body[:2], 16)
        phone_number = ubinascii.unhexlify(self._body[2:]).decode("gbk")
        self._body_data = {
            "flag": flag,
...
class T8401(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        set_type = int(self._body[:2], 16)
        total_concat = int(self._body[2:4], 16)
        concat_infos = self._body[4:]
        phonebook = []
...
class T8500(JTMessage):
    ...
    def body_from_hex(self):
        ...
        control_infos = []
        if self.is_version():
            control_num = int(self._body[:4], 16)
            control_data = self._body[4:]
            ...
            control_flag = bin(int(self._body[:2], 16))[2:]
...
class T8600(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        set_attr = int(self._body[:2], 16)
        total_area = int(self._body[2:4], 16)
        area_info = self._body[4:]
        area_data = []
    ...
    def body_from_hex(self):
        ...
        body_data:
            ...
            area_ids(list): empty list if all is 1
                item(int): area id
        """
        area_num = int(self._body[:2], 16)
        area_id_data = self._body[2:]
    ...
    def body_from_hex(self):
        ...
        body_data:
            ...
            route_ids(list): empty list if all is 1
                item(int): route id
        """
        route_num = int(self._body[:2], 16)
        route_id_data = self._body[2:]
...
class T8608(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        query_type = int(self._body[:2], 16)
        query_num = int(self._body[2:10], 16)
        query_id_info = self._body[10:]
...
class T0608(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{query_type}{data_len}{data}".format(**kwargs)
...
class T8700(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        cmd_word = int(self._body[:2], 16) if self._body[:2] else 0
        cmd_data = ""
        if self._body[2:]:
            cmd_data = ubinascii.unhexlify(self._body[2:])
        self._body_data = {
            "cmd_word": cmd_word,
            "cmd_data": cmd_data,
        }
...
class T0700(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{response_serial_no}{cmd_word}{cmd_data}".format(**kwargs)
...
class T0701(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{data_len}{data}".format(**kwargs)
...
class T0702(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{status}{time}{ic_read_result}{driver_name_len}{driver_name}{qualification_certificate_code}" \
...
class T0704(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{data_len}{data_type}{data}".format(**kwargs)
...
class T0705(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{data_len}{recive_time}{data}".format(**kwargs)
...
class T0800(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{media_id}{media_type}{media_encoding}{event_id}{channel_id}".format(**kwargs)
...
class T0801(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{media_id}{media_type}{media_encoding}{event_id}{channel_id}{loc_data}{media_data}".format(**kwargs)
    ...
    def body_from_hex(self):
        ...
        body_data:
            ...
            package_ids(list):
                item(int): package id
        """
        media_id = int(self._body[:8], 16)
        retransmission_num = int(self._body[8:10], 16)
        ids_data = self._body[10:]
...
class T8801(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        channel_id = int(self._body[:2], 16)
        shooting_order = int(self._body[2:6], 16)
        working_time = int(self._body[6:10], 16)
        save_flag = int(self._body[10:12], 16)
        resolution = int(self._body[12:14], 16)
        quality = int(self._body[14:16], 16)
        brightness = int(self._body[16:18], 16)
        contrast = int(self._body[18:20], 16)
        saturation = int(self._body[20:22], 16)
        chroma = int(self._body[22:24], 16)
        self._body_data = {
...
class T0805(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{response_serial_no}{result}{ids_num}{ids}".format(**kwargs)
...
class T8802(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        media_type = int(self._body[:2], 16)
        channel_id = int(self._body[2:4], 16)
        event_id = int(self._body[4:6], 16)
        start_time = self._body[6:18]
        end_time = self._body[18:30]
        self._body_data = {
...
class T0802(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{response_serial_no}{media_data_len}{media_data}".format(**kwargs)
...
class T8803(JTMessage):
    ...
    def body_from_hex(self):
        ...
        media_type = int(self._body[:2], 16)
        channel_id = int(self._body[2:4], 16)
        event_code = int(self._body[4:6], 16)
        start_time = self._body[6:18]
        end_time = self._body[18:30]
        delete_flag = int(self._body[30:32], 16)
        self._body_data = {
            "media_type": media_type,
            "channel_id": channel_id,
    ...
    def body_from_hex(self):
        ...
        body_data:
            ...
            audio_sample_rate(int):
                ...
                3 - 32K
        """
        recording_cmd = int(self._body[:2], 16)
        recording_time = int(self._body[2:6], 16)
        save_flag = int(self._body[6:8], 16)
        audio_sample_rate = int(self._body[8:10], 16)
        self._body_data = {
...
class T8805(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        media_id = int(self._body[:8], 16)
        delete_flag = int(self._body[8:10], 16)
        self._body_data = {
            "media_id": media_id,
            "delete_flag": delete_flag,
        }
...
class T8900(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        data_type = int(self._body[:2], 16)
        data = ubinascii.hexlify(self._body[2:]).decode("gbk")
        self._body_data = {
            "data_type": data_type,
            "data": data,
        }
...
class T0900(JTMessage):
    ...
    def body_to_hex(self):
        ...
        self._body = "{data_type}{data}".format(**kwargs)
...
class T8A00(JTMessage):
    ...
    def body_from_hex(self):
        ...
        """
        e = int(self._body[:8], 16)
        n = int(self._body[8:], 16)
        self._body_data = {
            "e": e,
            "n": n,
        }
